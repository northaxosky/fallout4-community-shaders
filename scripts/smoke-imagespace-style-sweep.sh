#!/bin/bash
# Imagespace style sweep: passthrough baseline (0) / Subtle (1) / Standard (2) / Vivid (3) / Cinematic (4).
# Confirms each style produces a distinct visual signature against the passthrough baseline.
set -e
...
STYLE_MARKER="$PLUGIN_DIR/.imagespace_force_style"
...
write_style()    { mkdir -p "$PLUGIN_DIR"; printf '%s' "$1" > "$STYLE_MARKER"; }
clear_markers()  { rm -f "$STYLE_MARKER" 2>/dev/null || true; }

run_smoke() {
    local label="$1" log_file="$2"
    echo "=== smoke-imagespace-style-sweep: $label ==="
    set +e
...
BASE="test-results/_sweep_style_passthrough.png"
SUB="test-results/_sweep_style_subtle.png"
STD="test-results/_sweep_style_standard.png"
VIV="test-results/_sweep_style_vivid.png"
CIN="test-results/_sweep_style_cinematic.png"
...
write_style 0 ; run_smoke "style=passthrough" "$LOG_BASE" ; capture passthrough "$(extract_results_dir "$LOG_BASE")" "$BASE"
write_style 1 ; run_smoke "style=subtle"      "$LOG_SUB"  ; capture subtle      "$(extract_results_dir "$LOG_SUB")"  "$SUB"
write_style 2 ; run_smoke "style=standard"    "$LOG_STD"  ; capture standard    "$(extract_results_dir "$LOG_STD")"  "$STD"
write_style 3 ; run_smoke "style=vivid"       "$LOG_VIV"  ; capture vivid       "$(extract_results_dir "$LOG_VIV")"  "$VIV"
write_style 4 ; run_smoke "style=cinematic"   "$LOG_CIN"  ; capture cinematic   "$(extract_results_dir "$LOG_CIN")"  "$CIN"
...
run_diff() {
    ...
    if [ -z "$dL" ]; then
        echo "STYLE-SWEEP VERDICT: SKIPPED (no dL in output)"
        return
    fi
    local absL
    ...
    if [ "$within" = "1" ]; then
        echo "STYLE-SWEEP VERDICT: PASS (|dL|=${absL}% within tolerance)"
    else
        echo "STYLE-SWEEP VERDICT: FAIL (|dL|=${absL}% outside tolerance)"
    fi
}

# Each style should diverge from passthrough by at least 1% luma. Pairwise comparisons across
# styles should also show non-trivial deltas (each one picks a different point in the look space).
echo
