...
namespace Util
{
	ID3D11DeviceChild* CompileShader(const wchar_t* a_filePath, const std::vector<std::pair<const char*, const char*>>& a_defines, const char* a_programType, const char* a_program)
	{
		...
		if (FAILED(D3DCompileFromFile(a_filePath, macros.data(), D3D_COMPILE_STANDARD_FILE_INCLUDE, a_program, a_programType, flags, 0, shaderBlob.put(), shaderErrors.put()))) {
		...
		else if (!_stricmp(a_programType, "vs_5_0")) {
		...
		else if (!_stricmp(a_programType, "hs_5_0")) {
		...
		else if (!_stricmp(a_programType, "ds_5_0")) {
		...
		else if (!_stricmp(a_programType, "cs_5_0")) {
		...
		else if (!_stricmp(a_programType, "cs_4_0")) {
			ID3D11ComputeShader* regShader;
			DX::ThrowIfFailed(device->CreateComputeShader(shaderBlob->GetBufferPointer(), shaderBlob->GetBufferSize(), nullptr, &regShader));
			return regShader;
		}
