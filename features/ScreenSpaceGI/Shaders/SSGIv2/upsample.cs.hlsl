// SSGI v2 upsample.cs - depth-aware bilateral 2x/4x upsample.
// Port of upstream Skyrim CS @ bb6460db.
//
// Reference: pixelmager - https://gist.github.com/pixelmager/a4364ea18305ed5ca707d89ddc5f8743
// Depth weighing: https://www.ppsloan.org/publications/ProxyPG.pdf#page=5

#include "Common.hlsli"

Texture2D<half>  srcDepth      : register(t0);
Texture2D<half>  srcAo         : register(t1);  // half/quarter-res
Texture2D<half4> srcIlY        : register(t2);  // half/quarter-res
Texture2D<half2> srcIlCoCg     : register(t3);  // half/quarter-res
Texture2D<half4> srcGiSpecular : register(t4);  // half/quarter-res
...
[numthreads(8, 8, 1)] void main(const uint2 dtid : SV_DispatchThreadID)
{
...
#else
	// FULL_RES: nothing to upsample - direct pass-through.
	outAo[dtid]         = srcAo[dtid];
	outIlY[dtid]        = srcIlY[dtid];
	outIlCoCg[dtid]     = srcIlCoCg[dtid];
	outGiSpecular[dtid] = srcGiSpecular[dtid];
	return;
#endif
	...
	{
		float2 uv = (dtid + 0.5) * RcpFrameDim * OUT_FRAME_DIM * RcpTexDim;
