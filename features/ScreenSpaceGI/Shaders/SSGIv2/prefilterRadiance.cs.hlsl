// SSGI v2 prefilterRadiance.cs - port of upstream Skyrim CS @ bb6460db.
//
// 5-mip radiance pyramid via groupshared scratch. Structurally identical to
// prefilterDepths.cs.hlsl / prefilterNormal.cs.hlsl, just with a different
// per-mip filter (4-tap linear average instead of point/octahedral).
//
// MIT licensed (Intel XeGTAO).
...
float3 RadianceMIPFilter(float3 r0, float3 r1, float3 r2, float3 r3)
{
...
[numthreads(8, 8, 1)] void main(uint2 dispatchThreadID : SV_DispatchThreadID, uint2 groupThreadID : SV_GroupThreadID)
{
	...
	// MIP 0 - emit 2x2 quad from gather
