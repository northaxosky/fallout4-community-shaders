...
//
// Reads the SH2-encoded indirect-lighting buffers produced by the SSGI chain
// (texIlY = sh2-Y luminance, texIlCoCg = YCoCg chrominance), reconstructs an
// RGB irradiance value at the receiving pixel's world-space normal, and ADDS it
// into kDiffuseBuffer.
//
...
//
// Coordinate frame:
//   gi.cs.hlsl writes SH coefficients in WORLD space (it rotates the per-sample
//   horizon vector by CameraViewInverse before SphericalHarmonics::Evaluate). This
//   consumer matches by rotating the hemisphere-reconstructed view-space normal
//   into world space using the same CameraViewInverse before SH evaluation. Falling
//   back to identity (DrawSSGI hasn't captured the matrix yet) collapses both sides
//   to view space; the math stays consistent either way.
...
void main(uint3 dtid : SV_DispatchThreadID)
{
    ...
    // Hemisphere-reconstructed view-space normal (matches SSGI prefilterNormal pattern).
    ...
    // Rotate to world space so the SH evaluation basis matches the producer (gi.cs).
    float3 worldspaceNormal = normalize(mul((float3x3)CameraViewInverse, viewspaceNormal));
    ...
    float ssgiIlY = SHHallucinateZH3Irradiance(ilY, worldspaceNormal);
