...
- **Compiler parity vs runtime equivalence.** `verify-shader-roundtrip.ps1` consumes the `fallout4-re` conformance manifest and confirms the eight proven HLSL variants still produce their published fxc DXBC hashes. `ShaderCompileLighting` compile-checks the three shipping permutations without native evidence. `D3DCompileFromFile` (d3dcompiler_47) is the same compiler family but is not guaranteed byte-identical to fxc-on-Win10-SDK-26100; runtime behavior still needs the screenshot diff.
