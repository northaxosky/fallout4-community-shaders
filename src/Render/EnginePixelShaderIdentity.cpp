...
namespace cs::engine
{
	namespace
	{
		EnginePixelShaderIdentityReason MapLookupReason(
			EnginePixelShaderLookupCorrelationReason a_reason) noexcept
		{
			...
			case EnginePixelShaderLookupCorrelationReason::
				...
				return EnginePixelShaderIdentityReason::
					kProductionLookupHookUnavailable;
		...
		EnginePixelShaderIdentitySource LookupFailureSource(
			bool a_currentHasPsid) noexcept
		{
		...
		EnginePixelShaderIdentitySource a_source) noexcept
	{
		...
		case EnginePixelShaderIdentitySource::
			kLookupReturnAndCurrentWrapper:
		...
		EnginePixelShaderIdentityReason a_reason) noexcept
	{
		...
		case EnginePixelShaderIdentityReason::
			kProductionLookupHookUnavailable:
	...
	{
		const bool currentShouldHavePsid =
			CurrentStatusCarriesPsid(a_current.status);
		...
		if (currentShouldHavePsid != currentHasPsid) {
			return {
				...
				.reason = EnginePixelShaderIdentityReason::
					kCurrentWrapperObservationInvalid
			};
		}
		...
		if (a_lookup.status
			...
			return {
				.status = EnginePixelShaderIdentityStatus::kAmbiguous,
		...
		if (a_lookup.status
			...
			if (!currentHasPsid) {
				return {
					.status = EnginePixelShaderIdentityStatus::kMatched,
					.source =
						EnginePixelShaderIdentitySource::kLookupReturn,
			...
			if (*a_current.psid != lookupPsid) {
				return {
					.status = EnginePixelShaderIdentityStatus::kRejected,
					.source = EnginePixelShaderIdentitySource::
						kLookupReturnAndCurrentWrapper,
					.reason = EnginePixelShaderIdentityReason::
						kLookupCurrentWrapperMismatch
				};
		...
		if (currentHasPsid) {
			return {
				.status = EnginePixelShaderIdentityStatus::kKnown,
				.source = EnginePixelShaderIdentitySource::kCurrentWrapper,
				.reason = a_current.status
		...
		return {
			.status = EnginePixelShaderIdentityStatus::kUnavailable,
