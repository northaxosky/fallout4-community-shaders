...
| **`dynres` fields are runtime-split, NOT a 3-way `REL::ID`** | raw byte offsets: `kOG = {0xF88, 0xF8C, 0xFA8}` vs `kNGAE = {0xFB8, 0xFBC, 0xFE5}` (widthRatio, heightRatio, isActivated). On OG the CommonLibF4 named members overlap `create` and crash on resize — that's why the accessors exist | ❓·❓·✅ | Source / Unvalidated | **always use `dynres::` accessors, never read the RTM fields directly** | `cs::engine::dynres`, `src/Render/Engine.h` |
| Engine-state accessors (the sanctioned entry points) | `GetSceneDepthSRV()`, `GetRenderTargetSRV/RTV/UAV(RenderTarget)`, `TryGetCameraMatrices(out)`, `GetVerticalFOV()`, `dynres::GetWidthRatio/GetHeightRatio(rtm)` | ✅·✅·✅ (compile) / ❓·❓·✅ (behavior) | Source | prefer these over raw struct reads | `src/Render/Engine.h` |
...
| `NiCamera::viewFrustum` | `+0x160`, `NiFrustum` (l/r/t/b/near/far/ortho) — persistent, jitter-free world-projection source | ✅·✅·✅ | Source | rebuilt into a proj | `RE::NiCamera` (`viewFrustum`); `cs::engine::TryGetWorldSceneProjection`, `src/Render/ProjectionMath.h` `BuildPerspectiveFromFrustum` |
